          ...
          <div className="flex items-center justify-between py-2.5 gap-4">
            <div className="flex items-center gap-3">
              <p className="text-sm text-blue-900">
                <span className="font-semibold">{exhausted ? 'Trial ended' : 'Free trial'}</span>
                {' — '}{used} of {cap} assessments used
              </p>
              {!exhausted && (
                <div className="flex items-center gap-2">
                  <div className="w-20 h-1 bg-blue-100 rounded-full overflow-hidden">
                    <div className="h-1 bg-blue-400 rounded-full" style={{ width: `${Math.min(100, (used / cap) * 100)}%` }} />
                  </div>
                  <span className="text-xs text-blue-500">{remaining} left</span>
                </div>
              )}
            </div>
        ...
        {showOnboarding && (
          <div className={`flex items-center gap-4 py-2.5 flex-wrap ${isTrial ? 'border-t border-blue-100' : ''}`}>
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-xs text-blue-400">{completedCount}/{steps.length}</span>
              ...
              {steps.map(step => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs transition-all ${
                    step.done ? 'text-blue-400 cursor-default' : 'bg-white border border-blue-200 hover:border-blue-400 cursor-pointer text-gray-700'
                  }`}
                >
                  {step.done
