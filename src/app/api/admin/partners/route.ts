...
export async function POST(req: NextRequest) {
  ...
  const {
    first_name, last_name, email, company, partner_type_id, password,
    override_commission_model, override_commission_value, override_commission_scope,
    override_commission_value_subsequent, override_commission_tiered_years,
    notes
  } = body;
  ...
  const { data, error } = await supabase.from("partners").insert({
    ...
    override_commission_value_subsequent: override_commission_value_subsequent ?? null,
    override_commission_tiered_years: override_commission_tiered_years ?? null,
  }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PATCH(req: NextRequest) {
  ...
  const allowed = [
    "first_name","last_name","email","company","partner_type_id","status","notes","bio",
    "override_commission_model","override_commission_value","override_commission_scope",
    "override_commission_value_subsequent","override_commission_tiered_years"
  ];
