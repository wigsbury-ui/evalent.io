      ...
      {students.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="mt-2 max-w-sm text-center text-sm text-gray-500">
              Register students to generate their unique assessment links and begin the admissions process.
            </p>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Click the report icon to view a student&apos;s assessment report.</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {students.map((student) => {
                    ...
                    return (
                      <tr key={student.id} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 font-mono text-xs text-gray-400">{student.student_ref}</td>
                        <td className="py-3">
                        ...
                        <td className="py-3">
                          {student.submission?.recommendation_band ? (
                            <Badge variant={student.submission.recommendation_band.toLowerCase().includes("ready") ? "success" : "secondary"}>
                              {student.submission.recommendation_band}
                            </Badge>
                          ) : (
                        ...
                        <td className="py-3">
                          {student.jotform_link && student.pipeline_status === "registered" ? (
                            <div className="flex items-center gap-1">
                              <a href={student.jotform_link} target="_blank" rel="noopener noreferrer" className="text-evalent-600 hover:text-evalent-700">
                            ...
                            <span className="text-gray-400 text-xs">
                              {student.pipeline_status !== "registered" ? "Done" : "—"}
                            </span>
                          )}
                        </td>
                        <td className="py-3">
                          {student.submission?.id &&
                          ["scored", "complete", "generating_report", "report_sent", "decided"].includes(student.pipeline_status) ? (
                            <a href={`/report?id=${student.submission.id}`} target="_blank" rel="noopener noreferrer" className="text-evalent-600 hover:text-evalent-700">
