...
function GradeChart({ data, gradeNaming }: { data: GradeBarData[]; gradeNaming: string }) {
  ...
  const segments: { key: keyof GradeBarData; color: string; label: string }[] = [
    ...
    { key: "accepted_support", color: "#6DBC7A", label: "Accepted (Support)" },
...
// ── Welcome Modal (Getting Started video — shown once per browser) ────────────
function WelcomeVideoModal({ url, onClose }: { url: string; onClose: () => void }) {
...
export default function SchoolDashboard() {
  ...
  useEffect(() => {
    const slug = data?.school?.name ? data.school.name.replace(/[^a-z0-9]/gi, '_').toLowerCase() : null;
    if (!slug) return;
