...
export default function SchoolActivityPage({ params }: { params: { id: string } }) {
  ...
  if (!data || data.error) return <div className="p-6 text-red-600">Error loading activity</div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          {Object.keys(stats.rec_counts).length === 0 ? (
            ...
            <div className="space-y-2">
              {Object.entries(stats.rec_counts).sort(([,a],[,b]) => (b as number)-(a as number)).map(([band, count]) => (
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wider">
            Recent Students ({stats.total_students} total)
          </h2>
        </div>
              ...
              {users.map((u: any) => (
                <div key={u.id} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    {u.last_sign_in_at && <p className="text-xs text-gray-300">{timeAgo(u.last_sign_in_at)}</p>}
                  </div>
                </div>
              ))}
            </div>
