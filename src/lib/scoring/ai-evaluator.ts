...
function getSystemPrompt(grade: number, locale: string): string {
  ...
  return `You are an experienced educational assessor evaluating extended writing for a Grade ${grade} admissions assessment. You evaluate both the content (relevance, depth, examples) and the quality of writing (organisation, sentence control, vocabulary, accuracy). You are warm but precise. You never comment on handwriting, only typed responses. You do not use bullet points. You write in ${langStyle}.
...
Writing prompt given to the student: "${task.prompt_text}"
...
export function generateMindsetNarrativePrompt(
  ...
  // Score bands from project brief Section 7.5
  let band: string;
...
export async function evaluateWriting(
  ...
  try {
    ...
    // Return a fallback evaluation rather than crashing the pipeline
    return {
      domain: task.domain,
      band: "Developing",
      score: 2,
