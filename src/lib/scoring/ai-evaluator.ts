...
Return JSON:
{
  ...
  "threshold_comment": "1 sentence on whether this response meets, exceeds, or falls below ${gradeLabel(task.grade, task.programme)} expectations for this domain."
}`;
}
...
export async function evaluateWriting(task: WritingTask): Promise<WritingEvaluation> {
  ...
  if (!task.student_response || task.student_response.trim().length < 10) {
    return {
      ...
      threshold_comment: "Does not meet minimum requirements for " + gradeLabel(task.grade, task.programme) + " assessment.",
    };
  }
