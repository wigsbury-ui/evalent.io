/**
 * Evalent PDF Report Generator — v2 (paragraph-aware narratives)
 *
 * Generates an 11-page admissions report as HTML.
 * AI narratives are now split into proper <p> paragraphs.
 */
...
function thresholdComment(domain: string, delta: number): string {
  if (delta >= 0)
    return `${domain} performance met the expected threshold.`;
  return `${domain} performance was below the expected threshold by ${Math.abs(delta).toFixed(1)} percentage points.`;
}

/**
 * Convert AI narrative text with newlines into proper HTML paragraphs.
 * Splits on double-newlines first, then falls back to splitting long
 * single blocks into sentence-grouped paragraphs.
 */
function narrativeToParagraphs(text: string): string {
  if (!text) return "";

  // Split on double newlines (AI sometimes returns paragraph breaks)
  let paragraphs = text.split(/\n\n+/).map((p) => p.trim()).filter((p) => p.length > 0);

  // If only one paragraph and it's long, try to split on single newlines
  if (paragraphs.length === 1 && paragraphs[0].length > 200) {
    const singleSplit = paragraphs[0].split(/\n/).map((p) => p.trim()).filter((p) => p.length > 0);
    if (singleSplit.length > 1) {
      paragraphs = singleSplit;
    }
  }

  // If still one very long block (>400 chars), split roughly in half at a sentence boundary
  if (paragraphs.length === 1 && paragraphs[0].length > 400) {
    const text = paragraphs[0];
    const mid = Math.floor(text.length / 2);
    // Find the nearest period after the midpoint
    const splitAt = text.indexOf(". ", mid);
    if (splitAt > 0 && splitAt < text.length - 50) {
      paragraphs = [
        text.substring(0, splitAt + 1).trim(),
        text.substring(splitAt + 2).trim(),
      ];
    }
  }

  return paragraphs
    .map((p) => `<p style="margin-bottom:10px; text-align:justify;">${p}</p>`)
    .join("\n");
}
...
function getStyles(): string {
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page { width: 210mm; min-height: 297mm; padding: 15mm 20mm 20mm 20mm; page-break-after: always; position: relative; }
    ...
    .cover-title { text-align: center; font-size: 22pt; font-weight: bold; color: ${COLORS.primary}; margin: 8px 0 4px; }
    .cover-subtitle { text-align: center; font-size: 10pt; color: ${COLORS.muted}; margin-bottom: 12px; }
    .recommendation-label { text-align: center; font-size: 14pt; font-weight: bold; color: ${COLORS.primary}; margin-top: 16px; text-transform: uppercase; letter-spacing: 1px; }
    .recommendation-band { text-align: center; font-size: 28pt; font-weight: 900; color: ${COLORS.primary}; margin: 4px 0 16px; }
    ...
    .section-title { font-size: 16pt; font-weight: bold; color: ${COLORS.primary}; text-align: center; margin: 12px 0 16px; text-transform: uppercase; letter-spacing: 0.5px; }
    table.snapshot { width: 100%; border-collapse: collapse; margin: 12px 0; font-size: 9.5pt; }
    table.snapshot th { background: ${COLORS.primary}; color: white; padding: 6px 8px; text-align: center; font-weight: 600; }
    table.snapshot th:first-child, table.snapshot td:first-child { text-align: left; }
    table.snapshot td { padding: 6px 8px; border-bottom: 1px solid ${COLORS.border}; text-align: center; }
    table.snapshot tr:nth-child(even) { background: ${COLORS.lightGray}; }
    table.snapshot td:last-child { text-align: left; font-size: 9pt; }
    table.lens { width: 100%; border-collapse: collapse; margin: 8px 0; font-size: 9.5pt; }
    table.lens td { padding: 8px; border: 1px solid ${COLORS.border}; vertical-align: top; }
    table.lens td:first-child { width: 110px; font-weight: bold; background: ${COLORS.lightGray}; text-align: right; padding-right: 12px; }
    ...
    .score-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin: 12px 0; }
    .score-box { text-align: center; padding: 12px; border: 1px solid ${COLORS.border}; border-radius: 6px; }
    .score-box .label { font-size: 9pt; color: ${COLORS.muted}; margin-bottom: 4px; }
    .score-box .value { font-size: 18pt; font-weight: bold; }
    .writing-band { display: inline-block; padding: 4px 12px; border-radius: 16px; font-weight: bold; font-size: 10pt; color: white; }
    .student-writing { background: ${COLORS.lightGray}; padding: 16px; border-radius: 6px; margin: 12px 0; font-size: 10pt; line-height: 1.7; white-space: pre-wrap; word-wrap: break-word; }
...
function page1_Cover(data: ReportInput): string {
  ...
  <div class="page">
    ...
    ${data.recommendation_narrative ? `<div class="narrative">${narrativeToParagraphs(data.recommendation_narrative)}</div>` : ""}
    ...
    <table class="lens">
      <tr>
        ...
        <td>${data.creativity
          ? `In the extended Creativity task, the response was graded at band ${data.creativity.band} and a score of ${data.creativity.score.toFixed(1)} / 4. ${data.creativity.narrative}`
          : "Not assessed"}</td>
      </tr>
      <tr>
        <td>Values Lens</td>
        <td>${data.values
          ? `In the extended Values task, the response was graded at band ${data.values.band} and a score of ${data.values.score.toFixed(1)} / 4. ${data.values.narrative}`
          : "Not assessed"}</td>
      </tr>
      <tr>
        <td>Mindset Lens</td>
...
function pageDomainAssessment(
  ...
  <div class="page">
    ...
    <div class="narrative">
      <p><b>Score breakdown:</b> MCQ ${domainData.mcq_pct.toFixed(1)}% (${domainData.mcq_correct}/${domainData.mcq_total} correct)${domainData.writing_score !== null ? `, Writing ${essayPct}% (${domainData.writing_band}, ${domainData.writing_score.toFixed(1)}/4)` : ""}. Combined: ${domainData.combined_pct.toFixed(1)}% against a threshold of ${domainData.threshold.toFixed(1)}%.</p>
    </div>
    ${domainData.writing_narrative ? `
    <div class="narrative">
      <p style="margin-bottom:6px;"><b>Writing evaluation:</b></p>
    ...
    <div class="narrative" style="color:${deltaColor(domainData.delta)}; font-weight:bold;">
      <p>${domainData.comment || thresholdComment(label, domainData.delta)}</p>
    </div>
  </div>
  `;
}
